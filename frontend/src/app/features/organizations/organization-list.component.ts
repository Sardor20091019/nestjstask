  ...
  template: `
    <div class="page-container flex flex-col h-full py-8 relative">
      ...
      <div class="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="page-title text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Organizations</h1>
          <p class="page-subtitle text-sm text-slate-500 dark:text-slate-400">Manage business units and organizational structures.</p>
        </div>
        <button 
        ...
        @if (loading()) {
          ...
          <div class="card flex flex-col items-center justify-center p-16 text-center max-w-2xl mx-auto mt-12 border-dashed bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs">
            <div class="rounded-full bg-brand-50 dark:bg-brand-950/40 p-6 mb-6 text-brand-500">
              <svg class="h-12 w-12" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21h18"/><path d="M9 8h1"/><path d="M9 12h1"/><path d="M9 16h1"/><path d="M14 8h1"/><path d="M14 12h1"/><path d="M14 16h1"/><path d="M5 21V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16"/></svg>
            </div>
            <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-2">No organizations found</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-8 max-w-md">Get started by setting up your first business unit or organization.</p>
      ...
      @if (isDrawerOpen()) {
        ...
        <div class="fixed inset-y-0 right-0 z-50 w-full max-w-md transform bg-white shadow-2xl transition-transform duration-300 ease-in-out dark:bg-slate-900 flex flex-col">
          <div class="px-6 py-6 sm:px-8 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <div>
              <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Create New Organization</h2>
              <p class="text-sm text-slate-500 dark:text-slate-400">Set up a business unit or workspace.</p>
            </div>
          ...
          <div class="flex-1 overflow-y-auto px-6 py-6 sm:px-8">
            <form [formGroup]="form" id="org-form" (ngSubmit)="submitOrganization()" class="space-y-6">
              <div>
                <label for="name" class="label text-sm font-medium text-slate-700 dark:text-slate-300">Organization Name <span class="text-rose-500">*</span></label>
                <input type="text" id="name" formControlName="name" class="input mt-1.5 w-full" placeholder="e.g. somethinadgintefrestinghere" />
                @if (form.controls.name.touched && form.controls.name.errors?.['required']) {
                  <p class="mt-1 text-xs text-rose-500">Organization name is required.</p>
                }
              </div>

              <div>
                <label for="created_by" class="label text-sm font-medium text-slate-700 dark:text-slate-300">Created By ID <span class="text-rose-500">*</span></label>
          ...
          <div class="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-3">
            ...
            <button type="submit" form="org-form" class="btn btn-primary" [disabled]="form.invalid || submitting()">
              @if (submitting()) { Creating... } @else { Create Organization }
            </button>
          </div>
        </div>
      }
...
export class OrganizationListComponent implements OnInit {
  ...
  private fetchOrganizations(): void {
    ...
    this.http.post<any>('/organizations/findAll', { limit: 20 })
      .pipe(
  ...
  submitOrganization(): void {
    ...
    this.http.post<any>('/organizations/create', payload)
      .pipe(
        takeUntilDestroyed(this.destroyRef),
        catchError((err) => {
