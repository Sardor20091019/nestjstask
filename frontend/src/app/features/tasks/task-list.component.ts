...
@Component({
  ...
  imports: [FormsModule, DatePipe, TaskStatusBadgeComponent, TaskDrawerComponent],
    ...
    <div class="page-container flex flex-col h-full py-8 relative">
      ...
      <div class="page-header">
        ...
        <div class="flex items-center gap-3">
          <button class="btn btn-primary shadow-brand-500/25" (click)="isDrawerOpen.set(true)">
      ...
      <div class="mt-6 flex flex-col sm:flex-row flex-wrap items-center gap-3">
        ...
        <div class="hidden md:flex items-center gap-1.5 p-1 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 ml-auto opacity-70 cursor-not-allowed" title="Priority sorting is simulated">
          <span class="text-xs font-semibold text-slate-500 pl-2 pr-1 uppercase tracking-wider">Priority:</span>
          <button class="px-3 py-1 rounded-lg text-sm font-medium bg-white shadow-sm text-slate-700 dark:bg-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700" disabled>All</button>
          <button class="px-3 py-1 rounded-lg text-sm font-medium text-slate-500 hover:text-slate-700 dark:text-slate-400" disabled>High</button>
          <button class="px-3 py-1 rounded-lg text-sm font-medium text-slate-500 hover:text-slate-700 dark:text-slate-400" disabled>Low</button>
        </div>
      </div>
      ...
      <div class="card flex-1 flex flex-col min-h-0 overflow-hidden mt-6">
        ...
        <div class="flex-1 overflow-auto">
          <table class="w-full text-left text-sm whitespace-nowrap">
            <thead class="sticky top-0 z-10 bg-white/90 backdrop-blur-md dark:bg-slate-900/90 shadow-sm border-b border-slate-200/60 dark:border-slate-800/60">
              ...
              @if (loading()) {
                @for (i of [1,2,3,4,5]; track i) {
                  <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td class="px-6 py-4 text-right"><div class="skeleton h-8 w-8 rounded-full ml-auto"></div></td>
                  </tr>
                }
              } @else {
                  ...
                  <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors cursor-pointer">
                    ...
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-1.5 text-slate-600 dark:text-slate-400">
                        <svg class="h-4 w-4 opacity-70" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>
                        {{ task.due_date | date: "MMM d, y" }}
                      </div>
                    </td>
                    <td class="px-6 py-4">
                      <span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-400 ring-1 ring-inset ring-slate-500/10 dark:ring-slate-400/20">
                        Project #{{ task.project_id }}
                      </span>
                    </td>
                    <td class="px-6 py-4 text-right">
                      <div class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-brand-100 to-brand-200 text-brand-700 ring-2 ring-white dark:from-brand-900 dark:to-brand-800 dark:text-brand-300 dark:ring-slate-900 font-bold text-xs shadow-sm">
                        {{ task.worker_user_id }}
                      </div>
                    </td>
                  </tr>
                    ...
                    <td colspan="5" class="px-6 py-16 text-center">
                      <div class="flex flex-col items-center justify-center">
                        <div class="rounded-full bg-slate-100 dark:bg-slate-800 p-4 mb-4">
                          <svg class="h-8 w-8 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="m9 12 2 2 4-4"/></svg>
                        </div>
                        <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">No tasks found</h3>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Try adjusting your search filters or create a new task.</p>
                      </div>
                    </td>
                  </tr>
                }
        ...
        <!-- Pagination Footer -->
        <div class="flex items-center justify-between border-t border-slate-200/60 bg-slate-50/50 px-6 py-4 dark:border-slate-800/60 dark:bg-slate-900/50">
          <p class="text-sm text-slate-500 dark:text-slate-400 font-medium">
            Showing page <span class="text-slate-900 dark:text-slate-200">{{ page().paginationinfo.page }}</span> of <span class="text-slate-900 dark:text-slate-200">{{ maxPages() }}</span>
            <span class="mx-2 text-slate-300 dark:text-slate-700">|</span>
            <span class="text-slate-900 dark:text-slate-200">{{ page().paginationinfo.total }}</span> total tasks
          </p>
          <div class="flex gap-2">
            <button class="btn btn-secondary btn-ghost" (click)="previousPage()" [disabled]="pageNumber() === 1 || loading()">
            ...
            <button class="btn btn-secondary btn-ghost" (click)="nextPage()" [disabled]="pageNumber() >= maxPages() || loading()">
              Next
...
export class TaskListComponent {
  ...
  private fetch() {
    ...
    return this.tasks
      .findAll(filters.search.trim(), options)
      .pipe(
        tap((response) => {
