        ...
        @if (loading()) {
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @for (i of [1, 2, 3]; track i) {
              <div
                class="card p-6 min-h-[140px] flex flex-col gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs"
              >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @for (user of users(); track user.id) {
              <div
                class="card p-6 flex flex-col justify-between group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs transition-all hover:shadow-md"
              >
                <div>
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-3">
                      <div
                        class="grid size-10 place-items-center rounded-xl bg-brand-50 dark:bg-brand-950/40 text-brand-600 dark:text-brand-400 font-bold text-base shadow-xs"
                      >
          ...
          >
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-8 max-w-md">
              Get started by creating a new system user account.
            </p>
            <button
          ...
          >
            <div>
              ...
              <p class="text-sm text-slate-500 dark:text-slate-400">
                Configure username and authorization role.
              </p>
            </div>
            <button
              type="button"
            ...
            >
              ...
              <div>
                <label
                  for="role"
                  class="label text-sm font-medium text-slate-700 dark:text-slate-300"
                  >Role Level <span class="text-rose-500">*</span></label
                >
                <select
                  id="role"
                  formControlName="role"
                  class="input mt-1.5 w-full bg-white dark:bg-slate-900"
                >
...
export class UserListComponent implements OnInit {
  ...
  submitUser(): void {
    ...
    const request$ =
      this.isEditMode() && this.editingUserId !== null
